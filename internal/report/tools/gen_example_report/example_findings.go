...
func exampleMiscFindings(base string) []model.Finding {
	return []model.Finding{
		{
			FindingID: "f-009", RuleID: "wapiti:content-security-policy-configuration", Category: "content security policy configuration",
			Title: "Wapiti: Content Security Policy Configuration",
			Description: `CSP "default-src" value is not safe`,
			Severity: model.SeverityMedium, LifecycleStatus: model.LifecycleConfirmed,
			LocationKey: "GET " + exampleURL("/dashboard"),
		},
		{
			FindingID: "f-010", RuleID: "cors-misconfig", Category: "wapiti",
			Title: "Wapiti: CORS Misconfiguration",
			Description: "Небезопасная конфигурация CORS: Access-Control-Allow-Origin: *.",
			Severity: model.SeverityHigh, LifecycleStatus: model.LifecycleConfirmed,
			LocationKey: "GET " + exampleURL("/api/"),
		},
		{
			FindingID: "f-011", RuleID: "CVE-2024-12345", Category: "nuclei",
			Title: "Known CVE in exposed component",
			Description: "Обнаружена известная уязвимость CVE-2024-12345 (CWE-79) в публичном эндпоинте.",
			Severity: model.SeverityCritical, LifecycleStatus: model.LifecycleConfirmed,
			LocationKey: "GET " + exampleURL("/api/v1/auth/login"),
		},
		{
			...
			Severity: model.SeverityLow, LifecycleStatus: model.LifecycleConfirmed,
			LocationKey: "GET " + base + "/",
		},
		{
			FindingID: "f-013", RuleID: "robots-txt", Category: "nuclei",
			...
			LocationKey: "GET " + base + "/robots.txt",
		},
		{
			FindingID: "f-014", RuleID: "10027", Category: "zap",
			...
			LocationKey: "GET " + base + "/assets/index-vJDItvOT.js",
		},
	}
}
