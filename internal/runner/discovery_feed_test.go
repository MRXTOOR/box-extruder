...
func TestIsAttackPayloadURL_NormalURLs(t *testing.T) {
	normal := []string{
		"https://sfera.release.dev.sfera-t1.ru/api/tenant/v1/user/routes",
		"https://sfera.release.dev.sfera-t1.ru/api/common/widgets/v1/maintenance-mode?app=PPOR",
		"https://sfera.release.dev.sfera-t1.ru/api/search?query=dashboard",
		"https://example.com/app/orchestration/agents?status=ONLINE&poolIds=&withoutPool=false",
...
func TestIsAttackPayloadURL_NoQuery(t *testing.T) {
	// URLs without query strings are never attack payloads.
	if isAttackPayloadURL("https://example.com/api/users") {
		t.Fatal("URL without query should not be attack payload")
	}
}
